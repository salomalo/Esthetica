...
class DB {
    ...
    private function action($action, $table, $where = array(), $orderby = '', $innerJoin = '') {
        if (count($where) === 3) {
            $operators = array('=', '>', '<', '>=', '<=');
            ...
            if (in_array($operator, $operators)) {
                $sql = "{$action} FROM {$table} {$innerJoin} WHERE {$field} {$operator} ? {$orderby}";
    ...
    public function get($table, $where = array(), $data = '*', $orderby = '', $innerJoin = '') {
        return $this->action('SELECT ' . $data, $table, $where, $orderby, $innerJoin);
    }
