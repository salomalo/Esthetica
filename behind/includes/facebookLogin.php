				...
				try	{
								...
								if	($check->exists())	{
												if	($check->data()->status	===	"Banni")	{
																Session::flash('flash',	array('status'	=>	'error',	'message'	=>	'<strong>Erreur!</strong> Votre compte a été banni. Veuillez nous contacter par téléphone pour plus d\'informations.'));
																Redirect::to('accueil');
												}
												...
												if	($login)	{
																...
																Session::flash('flash',	array('status'	=>	'success',	'message'	=>	'<strong>Succès!</strong> Bienvenue '	.	$fields['firstName']	.	', vous vous êtes connecté à votre compte avec succès.'));
																$rdv	=	$_GET['rdv'];
